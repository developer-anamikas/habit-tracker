...
export default function HabitAnalyticsCard({ habit, rangeType, onToggleToday }) {
  ...
  return (
    <article className="rounded-[20px] border border-surface-200 bg-white p-5 shadow-sm shadow-surface-200/60 transition-transform duration-200 hover:-translate-y-0.5">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          {habit.goal && (
            <p className="mt-2 line-clamp-2 text-[13px] text-surface-400">{habit.goal}</p>
          )}
        </div>
            ...
            {habit.timeline.map((day) => (
              <button
                ...
                className={`group flex flex-col items-center gap-2 rounded-2xl border border-transparent px-1.5 py-2 transition ${
                  day.isToday && day.isScheduled && !day.isFuture && canToggleToday
                    ? "cursor-pointer hover:border-accent-200 hover:bg-accent-50/60"
                    : "cursor-default"
                }`}
              >
                ...
                <span
                  className={`flex h-10 w-10 items-center justify-center rounded-full border text-[12px] font-semibold transition-all ${getTimelineClasses(
                    day,
                  )}`}
                >
                  {day.dayNumber}
        ...
        <div className="mb-2 flex items-center justify-between text-[12px] text-surface-500">
          <span>
            {habit.totalScheduledDays === 0
              ? "Not scheduled in this range"
              : `${habit.completedScheduledDays} of ${habit.totalScheduledDays} scheduled days completed`}
          </span>
          <span className={habit.tone.textClass}>
            {habit.completionPercentage === null ? "Not scheduled" : `${habit.completionPercentage}%`}
          </span>
        </div>

        <div className="h-2 overflow-hidden rounded-full bg-surface-100">
          <div
            className={`h-full rounded-full transition-[width] duration-500 ease-out ${habit.tone.barClass}`}
